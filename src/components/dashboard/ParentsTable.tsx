...
export default function ParentsTable() {
  ...
  const openEdit = (parent: ParentData) => {
  ...
  const onEdit = async (values: UserUpdateFormValues) => {
    ...
    setIsMutating(true);
    setServerError(null);
    const res = await updateParent(selectedParent.id, selectedParent.user_id, values);
  ...
  const openResetPassword = (parent: ParentData) => {
    setSelectedParent(parent);
    setServerError(null);
  ...
  const onResetPassword = async (values: ResetPasswordFormValues) => {
    ...
    const res = await resetUserPassword(selectedParent.user_id, values.password);

    if (res.error) {
      setServerError(res.error);
    } else {
      setResetSuccessMessage(`Password untuk ${selectedParent.users.name} berhasil diperbarui!`);
      setTimeout(() => {
  ...
  const openDelete = (parent: ParentData) => {
    setSelectedParent(parent);
  ...
  const onDelete = async () => {
    if (!selectedParent) return;
    setIsMutating(true);
    const res = await deleteParent(selectedParent.id, selectedParent.user_id);
  ...
  return (
    <div className="space-y-4">
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="border-white/10 bg-slate-900 text-white">
                        <DropdownMenuItem onClick={() => openEdit(parent)} className="focus:bg-white/10 focus:text-white cursor-pointer">
                          <Edit className="mr-2 h-4 w-4" /> Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => openResetPassword(parent)} className="focus:bg-yellow-500/20 focus:text-yellow-400 text-yellow-400 cursor-pointer">
                          <KeyRound className="mr-2 h-4 w-4" /> Ubah Password
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => openDelete(parent)} className="focus:bg-red-500/20 focus:text-red-400 text-red-400 cursor-pointer">
      ...
      <Dialog open={isResetPasswordOpen} onOpenChange={setIsResetPasswordOpen}>
        <DialogContent className="sm:max-w-[425px] bg-slate-900 border-white/10 text-white">
          <DialogHeader>
            <DialogTitle>Ubah Password Orang Tua</DialogTitle>
          </DialogHeader>
          <form onSubmit={resetPasswordForm.handleSubmit(onResetPassword)} className="space-y-4 mt-4">
            <p className="text-sm text-white/60">
              Mengubah password untuk akun <strong>{selectedParent?.users.name}</strong> ({selectedParent?.users.email}).
            </p>
            <div className="space-y-2">
        ...
        <AlertDialogContent className="bg-slate-900 border-white/10 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/60">
              Tindakan ini tidak dapat dibatalkan. Data orang tua <strong>{selectedParent?.users.name}</strong> akan dihapus permanen dari sistem.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
